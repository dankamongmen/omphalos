...
prepare_caps(const cap_value_t *caparray,unsigned n){
	...
	if((cap = cap_get_proc()) == NULL){
		fwprintf(stderr,L"Couldn't acquire capabilities (%s?)\n",strerror(errno));
		return -1;
	}
	if(cap_clear(cap)){
		fwprintf(stderr,L"Couldn't reset capabilities (%s?)\n",strerror(errno));
		cap_free(cap);
		return -1;
	}
	if(cap_set_flag(cap,CAP_EFFECTIVE,sizeof(ourarray) / sizeof(*ourarray),ourarray,CAP_SET)){
		fwprintf(stderr,L"Couldn't prep e-capabilities (%s?)\n",strerror(errno));
		cap_free(cap);
		return -1;
	}
	if(cap_set_flag(cap,CAP_PERMITTED,sizeof(ourarray) / sizeof(*ourarray),ourarray,CAP_SET)){
		fwprintf(stderr,L"Couldn't prep p-capabilities (%s?)\n",strerror(errno));
		cap_free(cap);
		return -1;
	}
	if(n){
	...
	if(cap_set_proc(cap)){
		fwprintf(stderr,L"Couldn't preset process capabilities (%s?)\n",strerror(errno));
		cap_free(cap);
		return -1;
	}
	cap_free(cap);
...
drop_privs(const char *name,const cap_value_t *caparray,unsigned n){
	...
	if(prctl(PR_SET_KEEPCAPS,1)){
		fwprintf(stderr,L"Couldn't set PR_SET_KEEPCAPS (%s?)\n",strerror(errno));
		return -1;
	}
	...
	// Change our user ID
	if(setuid(pw->pw_uid)){
		fwprintf(stderr,L"Couldn't setuid to %u (%s?)\n",pw->pw_uid,strerror(errno));
		return -1;
	}
	if(n){
		if((cap = cap_get_proc()) == NULL){
			fwprintf(stderr,L"Couldn't get process capabilities (%s?)\n",strerror(errno));
			return -1;
		}
		// older cap_set_flag() is missing the const on its third argument :/
		if(cap_set_flag(cap,CAP_EFFECTIVE,n,(cap_value_t *)caparray,CAP_SET)){
			fwprintf(stderr,L"Couldn't set up capabilities (%s?)\n",strerror(errno));
			cap_free(cap);
			return -1;
		}
		if(cap_set_proc(cap)){
			fwprintf(stderr,L"Couldn't set process capabilities (%s?)\n",strerror(errno));
			cap_free(cap);
			return -1;
		}
		cap_free(cap);
...
int handle_priv_drop(const char *name,const cap_value_t *caparray,unsigned n){
	...
	if(cap_get_flag(cap,CAP_SETUID,CAP_EFFECTIVE,&val) || val != CAP_SET){
		fwprintf(stderr,L"Don't have CAP_SETUID; won't change UID to %s (try -u '')!\n",name);
		cap_free(cap);
		return -1;
	}
	cap_free(cap);
